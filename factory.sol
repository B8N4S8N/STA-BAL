...
contract StableCoinFactory {
    ...
    mapping(address => address) public stableCoinToPool;

    constructor() public {
        owner = msg.sender;
    }

    function createStableCoin(address _pool, address _oracle, uint256 _targetPrice) public {
        require(msg.sender == owner, "Only the owner can create stablecoins.");
        require(!isStableCoin[_pool], "Stablecoin for this pool already exists.");

        address newStableCoin = new StableCoin(_pool, _oracle, _targetPrice);
        isStableCoin[newStableCoin] = true;
        stableCoinToPool[newStableCoin] = _pool;
    }
    ...
    function destroyStableCoin(address _stableCoin) public {
        require(msg.sender == owner, "Only the owner can destroy stablecoins.");
        require(isStableCoin[_stableCoin], "Stablecoin does not exist.");

        // Destroy stablecoin instance
        StableCoin(_stableCoin).destroy();
        isStableCoin[_stableCoin] = false;
        delete stableCoinToPool[_stableCoin];
    }
}
...
contract StableCoinFactory {
address public owner;
mapping(address => StableCoin) public instances;
constructor() public {
    owner = msg.sender;
}

function createInstance(address _pool, address _oracle, uint256 _targetPrice) public {
    require(msg.sender == owner, "Only the owner can create new instances.");
    address newInstance = new StableCoin(_pool, _oracle, _targetPrice);
    instances[newInstance] = StableCoin(_pool, _oracle, _targetPrice);
}
