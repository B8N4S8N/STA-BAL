...
// Use address payable to accept ether payments
...
stableCoinProxies[_aToken] = stableCoin; // changed mapping to use _aToken as key instead of _balancerPool
...
// Add a guard condition to ensure that the stablecoin contract has been deployed before trying to kill it
require(address(stableCoin).isContract(), "No contract deployed at the stablecoin address");
stableCoin.kill();
}
...
// Add a guard condition to ensure that the stablecoin contract has been deployed before trying to kill it
require(address(stableCoin).isContract